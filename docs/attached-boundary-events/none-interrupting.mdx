...
Hint: In order to update the deadline model the request for a deadline extension as an attached interrupting boundary event (We could use a non-interrupting event as well but if we do so we would need to provide some addional code to update the job within the Camunda Engine.) A solution for the model can be found <a target="_blank" href="/assets/docs/attached-boundary-events/non-interrupting/process-attached-non-interrupting-boundary-event.bpmn" download="process-attached-non-interrupting-boundary-event.bpmn">here</a>.
